...
class MerchantPolicy:
    ...
    def state_key(self, sku: str, inventory: int, buyer_offer: float | None, buyer_max_price: float, margin: float, round_number: int, clv_score: float | None = None) -> str:
        state_list = [sku, inventory, self._bucket(buyer_offer), self._bucket(buyer_max_price), round(margin, 2), round_number]
        if clv_score is not None:
            state_list.append(self._bucket(clv_score))
        return json.dumps(state_list, separators=(",", ":"))
