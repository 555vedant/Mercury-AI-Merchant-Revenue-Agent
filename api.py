...
def get_customer_data(
    ...
    clv_score = round(
        total_profit * 0.3 + total_spend * 0.6 + total_orders * 10,
        2,
    )
...
def get_customer_history(
    ...
    return {
        "negotiations": [dict(row) for row in negotiations_rows],
...
def negotiate(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=(f"Product SKU not found: " f"{request.product_sku}"),
        ) from error

    customer_id = request.customer_id or str(uuid4())
    ...
    if policy_result is not None:
        audit_trail.record_policy_decision(
            ...
            attempted_price=(
                result["observation"].get("buyer_price")
                or result["observation"].get("seller_price")
            ),
            ...
            discount=(
                max(
                    ...
                    (
                        merchant.list_price
                        - (result["info"].get("agreed_price") or merchant.list_price)
                    )
                    / merchant.list_price,
                )
    ...
    if result["status"] == "agreed":
        ...
        audit_trail.record_negotiation_event(
            ...
            margin=result["info"].get("seller_revenue", {}).get("margin_rate"),
            amount=(
                result["info"].get("agreed_price")
                or result["observation"].get("buyer_price")
                or result["observation"].get("seller_price")
            ),
        )
    ...
    final_price = result["info"].get("agreed_price")
...
def create_payment(
    ...
    if policy_result is None:
        raise HTTPException(
            status_code=409,
            detail=("Payment requires an " "agreed negotiation."),
        )
    ...
    try:
        ...
        amount = float(policy_result.transaction_amount)

        revenue = negotiation["result"]["info"].get(
            "seller_revenue",
            {},
        )
...
async def razorpay_webhook(
    ...
    if not signature or not secret:
        raise HTTPException(
            status_code=400,
            detail=("Razorpay webhook signature " "configuration is missing."),
        )
